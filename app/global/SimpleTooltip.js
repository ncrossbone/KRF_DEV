Ext.define('KRF_DEV.global.SimpleTooltip', {
	...
	onMousemoveEvt: function(){
		...
		require(["esri/toolbars/draw",
			...
			me.mouseMoveObj = on(_krad.map, "mouse-move", function(evt){
				
				me.showThis(evt.pageX, evt.pageY);
			});
			
			me.mouseDownObj = on(_krad.map, "mouse-down", function(evt){
				
				me.hideThis();
			});
			
			me.mouseUpObj = on(_krad.map, "mouse-up", function(evt){
				
				me.showThis(evt.pageX, evt.pageY);
			});
			
			me.mouseOutObj = on(_krad.map, "mouse-out", function(evt){
