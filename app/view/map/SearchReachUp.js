 ...
 * 중권역, 댐/보 체크 */
Ext.define("KRF_DEV.view.map.SearchReachUp", {
	mbId: "",
	searchWithEvent: function(evt){
		...
		var mapClickEvt = _krad.mapClickEvt;
		...
		_krad.offMapClickEvt();
		 ...
		 // 심볼 그리기
		_krad.drawSymbol(evt);
		
		_rchLine.getFeaturesWithEvent(evt, function(features){
    ...
    searchUpWithFeature: function(lineFeature, cnt){
    	...
    	/* 초기화 시 검색 종료 */
    	if(_krad.searchStopCheck(cnt) == false){ return false };
    	
    	_krad.searchCnt++; // 검색 카운트 증가
    	...
    	// 본류 체크 여부
    	var isBonDraw = _krad.searchConfigInfoJson.isBonDraw;
    	// 지류 체크 여부
    	var isJiDraw = _krad.searchConfigInfoJson.isJiDraw;
    	// 중권역 체크 여부
    	var isMWDraw = _krad.searchConfigInfoJson.isMWDraw;
    	// 댐/보 체크 여부
    	var isDaemBoDraw = _krad.searchConfigInfoJson.isDaemBoDraw;
    	...
    	_rchArea.getFeaturesWithWhere("CAT_DID = '" + catDid + "'", function(areaFeatures){
			...
			if(isDaemBoDraw == false){
				...
				_rchNode.getFeaturesWithWhere("LD_RCH_DID = '" + rchDid + "'", function(ldNodeFeatures){
					
					// LD_RCH_DID : 노드 기준으로 하류 좌측 유출 아이디
	    			_rchNode.getFeaturesWithWhere("RD_RCH_DID = '" + rchDid + "'", function(rdNodeFeatures){
    ...
    drawGraphic: function(isDrawGraphic, lineFeature, areaFeatures){
		...
		if(isDrawGraphic == true){
    		...
    		/* 라인 그래픽 그리기 */
			_krad.drawGraphic(lineFeature, "reachLine");
			
			/* 집수구역 그래픽 그리기 */
			for(var i = 0; i < areaFeatures.length; i++){
				
				_krad.drawGraphic(areaFeatures[i], "reachArea");
			}
		}
    },
    searchUpWithWhere: function(isUpSearch, where, cnt){
    	...
    	if(isUpSearch == true){
			...
			// 상류 리치라인 조회
    		_rchLine.getFeaturesWithWhere(where, function(features){
