						...
						if(tmpIdx == -1){
				    		...
				    		if(isEndLD == true && isEndRD == true){
					    			...
					    			if(me.stRchIds.length > 0 && me.edRchIds.length > 0){
					    				...
					    				for(var i = 0; i < me.arrCommGrp.length; i++){
						    				...
						    				if(isGeoTrib == false){
						    					...
						    					//me.setReachUpLineTest(commRchDid, 0, false);
				    			...
				    			if(isPush == true){
					    			...
					    			if(me.drawOption == "startPoint"){
					    				me.arrDownGrpStart = me.arrDownGrp.length-1;
					    			}else if(me.drawOption == "endPoint"){
					    				me.arrDownGrpEnd = me.arrDownGrp.length-1;
					    			}*/
				    			}
				    		}
						}
			...
			queryTask.execute(query, function(featureSet){
				...
				if(featureSet.features.length > 0){
					...
					if(cnt == 0){
						...
						if(me.arrDownGrpStart == ""){
		    				me.arrDownGrpStart = 1;
		    			}else if(me.arrDownGrpEnd == ""){
		    				me.arrDownGrpEnd = 1;
		    			}
		    			...
		    			//console.info(me.arrDownGrpStart);
		    			for(var list = 0 ; list < me.arrDownGrp.length; list++){
		    				for(var arrList = 0 ; arrList < me.arrDownGrp[list].length; arrList ++){
		    					if(me.arrDownGrp[list][arrList].attributes.RCH_DID == me.cmRiRchDid){
		    						me.bonLine.push(me.arrDownGrp[list][arrList].attributes);
		    						
		    						if(me.arrDownGrp[list][arrList].attributes.GEO_TRIB == 0){
		    							me.isNotBon1 = false;
		    						}else{
		    							me.isNotBon1 = true;
		    						}
		    					...
		    					if(me.arrDownGrp[list][arrList].attributes.RCH_DID == me.cmLeRchDid){
		    						me.bonLine.push(me.arrDownGrp[list][arrList].attributes);
		    			...
		    			//me.isNotBon 본류가 있는지 없는지 판단
						
		    			
		    			//return;
		    			if(me.isNotBon == false){
		    				
		    			}else{

		    				for(var i = 0 ;i < me.arrDownGrp.length;i++){
								for(var j = 0 ; j < me.arrDownGrp[i].length; j++){
									if(me.arrDownGrp[i][j].attributes.RCH_DID == me.bonLine[0].RCH_DID){
										
										if(me.arrDownGrpStart == i){
											me.arrDownGrpStartBon = 0;
										}else if(me.arrDownGrpEnd == i){
											me.arrDownGrpEndBon = 0;
										}
									}
									
									if(me.arrDownGrp[i][j].attributes.RCH_DID == me.bonLine[1].RCH_DID){
										
										
										if(me.arrDownGrpStart == i){
											me.arrDownGrpStartBon = 1;
										}else if(me.arrDownGrpEnd == i){
											me.arrDownGrpEndBon = 1;
										}
									}
								}
							}
							...
							for(var a = 0;a < me.arrDownGrp[me.arrDownGrpEndBon].length;a++){
								if(me.arrDownGrp[me.arrDownGrpEndBon][a].attributes.GEO_TRIB == me.bonLine[me.arrDownGrpEndBon].GEO_TRIB){
									
									endArrayCut.push(a);
								}
							}
							
							for(var b = 0;b < me.arrDownGrp[me.arrDownGrpStart].length;b++){
								if(me.arrDownGrp[me.arrDownGrpStart][b].attributes.GEO_TRIB == me.bonLine[me.arrDownGrpStartBon].GEO_TRIB){
									
									startArrayCut.push(b);
								}
							}
							
							var point1bonRchDid = me.arrDownGrp[me.arrDownGrpEndBon][endArrayCut[0]].attributes.RCH_DID; // 하류를 따라가다 가장처음만나는 본류1
							var point2bonRchDid = me.arrDownGrp[me.arrDownGrpStart][startArrayCut[0]].attributes.RCH_DID; // 하류를 따라가다 가장처음만나는 본류2
							...
							me.arr2RRchDid = me.arrDownGrp[me.arrDownGrpEndBon][endArrayCut[0]].attributes.LU_RCH_DID;
							me.arr2LRchDid = me.arrDownGrp[me.arrDownGrpEndBon][endArrayCut[0]].attributes.RU_RCH_DID;
						...
						/*console.info(linefalse1);
						console.info(linefalse2);*/
					...
					if(evtType == "Reach"){
						...
						if(me.isNotBon){
							if(rchDid != me.falseDid1 && rchDid != me.falseDid2){
								...
								// 집수구역 그리기
								me.setReachArea(catDid);
							}else if(rchDid == me.stRchDids || rchDid == me.edRchDids){
								me.drawGraphic(feature, "reachLine");
								// 집수구역 그리기
								me.setReachArea(catDid);
							}else if(rchDid == me.stRchDids && rchDid == me.edRchDids){
