...
testResult = function(jsonData){
	...
	for(var a = 0 ; a < jsonData.data.length ; a++){
		if(a == 0){
			...
			//처음 배열 생성
			setChartListPush(jsonData.data[a] , rn , resultArray , count);
			...
			if(jsonData.data[a].RN != rn){
				...
				count++
				setResultDataPush(jsonData.data[a] , rn , resultArray, "Chart" , count);
		...
		if(rn == jsonData.data[a].RN_2){
			...
			count++
			setResultDataPush(jsonData.data[a] , rn , resultArray, "Data" , count);
		}
...
setChartListPush = function(dataList , rn , resultArray , count){
	...
	setResultDataPush(dataList, rn , resultArray, "" , count);
	
},


//검색결과에 데이터 Push
setResultDataPush = function(dataList , rn , resultArray , type , count){
	...
	if(type == "Data"){
		...
		for(var i = 0 ;  i < jsonData.length ; i++){
			...
			if(chartName != undefined){
				...
				var replace = count + dataList.CHART_DATE.replace(/\./g,'');
